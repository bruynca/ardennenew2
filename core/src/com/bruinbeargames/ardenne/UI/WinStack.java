...
public class WinStack  implements  Observer {
    ...
    public WinStack(ArrayList<Unit> arrUnitsIn,  Hex hex) {
  ...
  //        reCalculate();
    ...
    private void reCalculate(){
        ...
        for (Unit unit:arrWork){
            if (attack == null){
                attack = new Attack(hex,unit.isAllies,true,false, unit);
        ...
        /**
         *  hex clicked will delete image
         */
        if (arrWork.size() > 0) {
            MobileAssualt.instance.createCombatImage(attack.getHexTarget(), true);
        }else{
            cancel();
        }
    }
        ...
        private void end(){
            ...
            if (arrWork.size() == 0){
                ...
                MobileAssualt.instance.updateFromStack(arrWork);
            }
       }
        public void cancel() {
            window.remove();
